...
rule kofam_scan:
  output:
    tsv = "results/kofamscan_out_test/kofam_out_{CLUSTER}.tsv",
    txt = "results/kofamscan_out_test/kofam_out_{CLUSTER}.txt"
  input: expand("results/prokka_out_test/{{CLUSTER}}/{{CLUSTER}}.faa")
...
rule egg_for_decoder:
  output:  "results/eggnog_out_test/r2_egg_{CLUSTER}"
  input:  "results/eggnog_out_test/{CLUSTER}.in.emapper.annotations"
  script: "scripts/egg_regex.py"

rule egg_format_decoder:
  output:
    one = "results/eggnog_out_test/r2_egg_{CLUSTER}.txt",
    two = "results/eggnog_out_test/head_r2_egg_{CLUSTER}.txt"
  input: "results/eggnog_out_test/r2_egg_{CLUSTER}"
  shell:
    """
...
rule rename_egg_decoder:
  ...
  input:  "results/eggnog_out_test/head_r2_egg_{CLUSTER}.txt"
  shell: "sed 's/^/egg_/g' {input} > {output}"

rule rename_kofam_decoder:
  output:  "results/kofamscan_out_test/kofam_decoder_{CLUSTER}.txt"
  input:  "results/kofamscan_out_test/kofam_out_{CLUSTER}.txt"
  shell: "sed 's/^/kofam_/g'  {input} > {output}"

rule concat_for_decoder:
  output:  "results/decoder_out_test/concat_kofam_{CLUSTER}.txt"
  input:
    egg = "results/eggnog_out_test/head_r2_egg_{CLUSTER}.txt",
    kofam = "results/kofamscan_out_test/kofam_out_{CLUSTER}.txt"
  shell: "cat {input.egg} {input.kofam} > {output}.txt"

rule keggdecoder:
  output:  "results/decoder_out_test/decoder_out_{CLUSTER}"
  input:  "results/decoder_out_test/concat_kofam_{CLUSTER}.txt"
...
rule low_qual:
   output: "results/seqkit_test/low_qual_{CLUSTER}.faa"
   input:
...
rule kraken2:
  output: "results/krakren_test/kraken2_{CLUSTER}_out"
  input:  "results/seqkit_test/low_qual_{CLUSTER}.faa"
